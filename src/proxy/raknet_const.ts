...
export enum MessageIdentifiers
{
    ...
    /// , public key (if do security is true), MTU(2). If public key fails on client, returns PUBLIC_KEY_MISMATCH
    ...
    /// RakPeer - Same as ADVERTISE_SYSTEM, but intended for internal use rather than being passed to the user.
    /// Second byte indicates type. Used currently for NAT punchthrough for receiver port advertisement. See NAT_ADVERTISE_RECIPIENT_PORT
    ...
    /// RakPeerInterface::Receive() you will get SND_RECEIPT_ACKED or SND_RECEIPT_LOSS. The message will be 5 bytes long,
    /// and bytes 1-4 inclusive will contain a number in native order containing a number that identifies this message.
    /// This number will be returned by RakPeerInterface::Send() or RakPeerInterface::SendList(). SND_RECEIPT_ACKED means that
    /// the message arrived
    ...
    /// will be returned by RakPeerInterface::Send() or RakPeerInterface::SendList(). SND_RECEIPT_LOSS means that an ack for the
    /// message did not arrive (it may or may not have been delivered, probably not). On disconnect or shutdown, you will not get
    /// SND_RECEIPT_LOSS for unsent messages, you should consider those messages as all lost.
    ...
    TIMESTAMP = 0x1b,
    /// RakPeer - Pong from an unconnected system.  First byte is UNCONNECTED_PONG, second sizeof(RakNet::TimeMS) bytes is the ping,
    /// following bytes is system specific enumeration data.
    /// Read using bitstreams
    UNCONNECTED_PONG = 0x1c,
    /// RakPeer - Inform a remote system of our IP/Port. On the recipient, all data past ADVERTISE_SYSTEM is whatever was passed to
    /// the data parameter
    ADVERTISE_SYSTEM = 0x1d,
    // RakPeer - Downloading a large message. Format is DOWNLOAD_PROGRESS (MessageID), partCount (unsigned int),
    ...
    /// NATPunchthrough plugin: Destination system is not responding to NAT_GET_MOST_RECENT_PORT. Possibly the plugin is not installed.
    ...
    /// \note If you fail to connect to a system, this does not automatically mean you will get FCM2_VERIFIED_JOIN_FAILED as that system may have been shutting down from the host too
    /// \sa FullyConnectedMesh2::StartVerifiedJoin()
    FCM2_VERIFIED_JOIN_START = 0x57,
    /// \internal The client has completed processing for all systems designated in FCM2_VERIFIED_JOIN_START
    ...
    /// RakPeerInterface::CloseConnection() was automatically called for all systems connected due to FCM2_VERIFIED_JOIN_START
    ...
    /// The system that called StartVerifiedJoin() got FCM2_VERIFIED_JOIN_CAPABLE from the client and then called RespondOnVerifiedJoinCapable() with true
    ...
    /// The system that called StartVerifiedJoin() got FCM2_VERIFIED_JOIN_CAPABLE from the client and then called RespondOnVerifiedJoinCapable() with false
    /// CloseConnection() has been automatically called for each system connected to since FCM2_VERIFIED_JOIN_START.
    ...
    /// Read endpointGuid and port as per ROUTER_2_FORWARDING_ESTABLISHED
    ...
    ///  get TEAM_BALANCER_TEAM_ASSIGNED later.
    ...
    ///  you will get TEAM_BALANCER_SET_TEAM later.
...
export const RELIABLITY_MAP:{reliable?:boolean, ordered?:boolean, sequenced?:boolean, ack?:boolean}[] = [
    {},
    {ordered:true, sequenced:true},
    {reliable:true},
    {reliable:true, ordered:true},
    {reliable:true, ordered:true, sequenced:true},
    {ack:true},
    {reliable:true, ack:true},
    {reliable:true, ordered:true, ack:true},
];
